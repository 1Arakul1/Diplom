...
class OrderAdmin(admin.ModelAdmin):
    ...
    readonly_fields = ('track_number', 'order_date', 'user', 'email', 'delivery_option', 'payment_method', 'address', 'total_amount') # Поля только для чтения
    list_editable = ('status',)
    fieldsets = (
        ('Основная информация', {
            'fields': ('user', 'email', 'delivery_option', 'payment_method', 'address', 'total_amount', 'order_date', 'track_number')
        }),
        ('Статус заказа', {
            'fields': ('status',)
        }),
...
class CartItemAdmin(admin.ModelAdmin):
    list_display = ('user', 'build', 'cpu', 'gpu', 'quantity')
    list_filter = ('user',)
    search_fields = ('user__username',)

    def has_add_permission(self, request):
        return False #Запрещаем добавлять новые элементы

    def has_delete_permission(self, request, obj=None):
        return False #Запрещаем удалять элементы

...
class RAM(models.Model):
    ...
    def __str__(self):
        return f"{self.manufacturer} {self.model} ({self.capacity} ГБ, {self.frequency} МГц)"
...
class Storage(models.Model):
    ...
    def __str__(self):
        return f"{self.manufacturer} {self.model} ({self.capacity} ГБ, {self.type})"
...
class Cooler(models.Model):
    ...
    manufacturer = models.ForeignKey(Manufacturer, on_delete=models.CASCADE, verbose_name="Производитель")
    model = models.CharField(max_length=255, verbose_name="Модель")
    cooler_type = models.CharField(max_length=50, verbose_name="Тип охлаждения", choices=[
        ('air', 'Воздушное'),
        ('liquid', 'Жидкостное'),
    ])

...
def predict_fps(request):
    if request.method == 'POST':
        ...
        if not all([cpu_id, gpu_id, game_id, settings]):
            ...
            games = Game.objects.all()
            context = {'cpus': cpus, 'gpus': gpus, 'games': games, 'error_message': "Пожалуйста, заполните все поля."}
        ...
        try:
            ...
            fps_data = FPSData.objects.get(game=game, cpu=cpu, gpu=gpu, settings=settings)
            ...
            context = {'error_message': "Нет данных для этой конфигурации.", 'cpus': CPU.objects.all(), 'gpus': GPU.objects.all(),'games': Game.objects.all(),} #Отправляем сообщение об ошибке

        except ValueError as e:
            context = {'error_message': "Некорректные данные: {}. ".format(e), 'cpus': CPU.objects.all(), 'gpus': GPU.objects.all(),'games': Game.objects.all(),}
            return render(request, 'games/fps_form.html', context)
        
        context = {'game': game, 'cpu': cpu, 'gpu': gpu, 'predicted_fps': predicted_fps, 'settings': settings}
